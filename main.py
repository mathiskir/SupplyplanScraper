...
element_select = driver.find_element(By.XPATH, "/html/body/table/tbody/tr[4]/td/span/nobr/select")
select = Select(element_select)
select.select_by_visible_text("Druck-Kla")

calender_week_select= driver.find_element(By.XPATH, "/html/body/table/tbody/tr[2]/td/span/nobr/select")
...
def get_table_info(day, index): 
    table = driver.find_element(By.XPATH, f"/html/body/center/font[1]/div/p[{index}]/table/tbody")
    rows = table.find_elements(By.TAG_NAME, "tr")
    check_for_td = rows[0].find_elements(By.TAG_NAME, "td")
    if len(check_for_td) == 0: available = True
    ...
    if available:
        ...
        for replacement in table_data.values():
            if "9" in replacement[0]:
                if len(replacement_to_show) != 0:
                    if replacement_to_show[len(replacement_to_show)-1][0] == "9a, 9b,":
        ...
        for replacement in replacement_to_show:
            if not replacement[0] == " ":
                ...
                text+=f"Klasse/n: *{replacement[0]}*, Tag: *{replacement[3]}*; *{replacement[1]}*, Stunden: *{replacement[2]}*, Lehrer: *{replacement[5]}* unterrichet *{replacement[4]}*, Entfall: *{replacement_flag}*, Mitbetreuung: *{cosupport_flag}*, Mehr: *{replacement[9]}*;*{replacement[14]}*\n\n"
            else:
...
for result in results.items(): 
    for chatid in telegram_target_chatid:
        req = requests.post(f"{telegram_base_url}{telegram_bot_token}/sendMessage?chat_id={chatid}&text={result[0]}\n{result[1]}&parse_mode=markdown")
